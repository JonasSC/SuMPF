...
class SignalChain(object):
    def __init__(self):
        ...
        self.__divide = sumpf.modules.Divide()
        sumpf.connect(self.__record_fft.GetSpectrum, self.__divide.SetValue1)
        sumpf.connect(self.__playback_fft.GetSpectrum, self.__divide.SetValue2)
        self.__multiply = sumpf.modules.Multiply()
        sumpf.connect(self.__record_fft.GetSpectrum, self.__multiply.SetValue1)
        sumpf.connect(self.__regularization.GetOutput, self.__multiply.SetValue2)
        self.__select_regularization = sumpf.modules.SelectSpectrum()
        sumpf.connect(self.__multiply.GetResult, self.__select_regularization.SetInput1)
        sumpf.connect(self.__divide.GetResult, self.__select_regularization.SetInput2)
        self.__REGULARIZE = 1
        ...
        sumpf.connect(self.__copy_window.GetOutput, self.__apply_window.SetValue2)
        self.__bypass_window = sumpf.modules.SelectSignal()
        sumpf.connect(self.__bypass_window.SetInput1, self.__merge_uir.GetOutput)
        sumpf.connect(self.__bypass_window.SetInput2, self.__apply_window.GetResult)
        self.__BYPASS = 1
        self.__WINDOW = 2
        ...
        sumpf.connect(self.__copy_filter.GetOutput, self.__apply_filter.SetValue2)
        ...
        sumpf.connect(self.__bypass_filter.SetInput2, self.__apply_filter.GetResult)
        self.__FILTER = 2
    ...
    def __Save(self, merger, filename, file_format):
        ...
        if isinstance(data, sumpf.Signal):
            filehandler = sumpf.modules.SaveSignal()
            filehandler.SetSignal(data)
        else:
            filehandler = sumpf.modules.SaveSpectrum()
    ...
    def __Load(self, merger, id_dict, filename):
        ...
        if merger in [self.__merge_uir, self.__merge_pir]:
            filehandler = sumpf.modules.LoadSignal(filename)
            ...
            filehandler = sumpf.modules.LoadSpectrum(filename)
            filehandler.SetFilename(filename)
