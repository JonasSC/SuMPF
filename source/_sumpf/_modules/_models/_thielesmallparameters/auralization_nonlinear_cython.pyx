...
#	But somehow this implementation runs about four times slower than the pure
...
##	output[0, :, 3:] = numpy.asarray(voltage_channels, dtype=numpy.float64)
...
#			for c in range(number_of_channels):
#				for i in range(3):
#					output[s][c][i] = saved_samples[s][c][i]
#	for c in range(number_of_channels):
#		for i in range(3, length):
#			x = output[1][c][i - 1]
#			v = output[1][c][i - 1]
...
#			b_vector[0] = L * m
#			b_vector[1] = L * w + R * m
#			b_vector[2] = L * k + M * M + R * w
#			b_vector[3] = R * k
#			b = numpy.dot(b_matrix, b_vector)
#			output[1][c][i] = (sum(numpy.multiply(output[0][c][i-3:i+1], a)) - sum(numpy.multiply(output[1][c][i-3:i], b[0:3]))) / b[3]
#			# calculate the velocity, the acceleration, the current and the sound pressure
#			output[2][c][i] = K * (output[1][c][i] - output[1][c][i - 1]) - q * output[2][c][i - 1]
#			output[3][c][i] = K * (output[2][c][i] - output[2][c][i - 1]) - q * output[3][c][i - 1]
