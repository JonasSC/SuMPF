...
class TestSignalPlotPanel(BaseTestCase):
    ...
    def test_interactively(self):
        ...
        try:
            ...
            add = sumpf.modules.Add()
            sumpf.connect(rectify.GetOutput, add.SetValue1)
            sumpf.connect(shift.GetOutput, add.SetValue2)
            sumpf.connect(add.GetResult, plot.SetSignal)
            plot.LogarithmicX()
            plot.LogarithmicY()
