...
@hypothesis.given(before_length=hypothesis.strategies.integers(min_value=2, max_value=2 ** 8),
                  ...
                  decays=hypothesis.strategies.lists(elements=hypothesis.strategies.floats(min_value=5e-4, max_value=1.0 - 1e-6), min_size=1, max_size=5),  # pylint: disable=line-too-long
                  sampling_rate=tests.strategies.sampling_rates,
                  offset=hypothesis.strategies.integers(),
                  labels=tests.strategies.labels())
def test_reverberation_time(before_length, decay_length, after_length,
                            start_level, decays, sampling_rate, offset, labels):
    ...
    edc = _new_energy_decay_curve(before_length, decay_length, after_length,
                                  start_level, decays, sampling_rate, offset, labels)
...
@hypothesis.given(before_length=hypothesis.strategies.integers(min_value=2, max_value=2 ** 12),
                  ...
                  start_level=hypothesis.strategies.floats(min_value=1e-4, max_value=1e15),
                  decays=hypothesis.strategies.lists(elements=hypothesis.strategies.floats(min_value=1e-4, max_value=1.0), min_size=1, max_size=5),     # pylint: disable=line-too-long
                  sampling_rate=tests.strategies.sampling_rates,
                  offset=hypothesis.strategies.integers(),
                  labels=tests.strategies.labels())
def test_decay_model(before_length, decay_length, after_length,
                     start_level, decays, sampling_rate, offset, labels):
...
def _new_energy_decay_curve(before_length, decay_length, after_length,
                            start_level, decays, sampling_rate, offset, labels):
    ...
    for channel, decay in zip(channels, decays):
        ...
        channel[before_length:decay_stop] = start_level * numpy.exp((math.log(decay) / decay_length) * numpy.arange(decay_length))  # pylint: disable=line-too-long
